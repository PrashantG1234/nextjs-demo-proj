                        ...
                        switch (p.type) {
                          case "text":
                            return message.role === "user" ? (
                              <span key={partIndex} className="whitespace-pre-wrap">{p.text}</span>
                            ) : (
                              <MarkdownRenderer
                                key={partIndex}
                                content={p.text}
                          ...
                          case "reasoning":
                            return (
                              <div key={partIndex} className="mt-3 p-3 bg-white/50 rounded-md border border-neutral-200">
                                ...
                                <span className="text-sm text-neutral-700">{p.reasoning ?? p.text}</span>
                              </div>
                            );
                          case "step-start":
                          ...
                          default:
                            if (p.type === "dynamic-tool" || (typeof p.type === "string" && p.type.startsWith("tool-"))) {
                              ...
                              const isDone = p.state === "output-available" || p.state === "result";
                              return (
